...
#define NU_DEFAULT_FIELD_CAPACITY      128
...
typedef struct
{
    ...
    nu__slotlist_t archetype_field_lists;
    nu_ident_t    *archetype_names;
    nu__slot_t     first_archetype;

    nu__slotmap_t fields;
    nu_ident_t   *field_names;
...
{
    info->field_capacity      = NU_DEFAULT_FIELD_CAPACITY;
...
{
    NU_ASSERT(info->field_capacity > 0);
    ...
    nu__slotmap_init(alloc,
                     NU_MEMORY_USAGE_ECS,
                     sizeof(nu__field_entry_t),
                     info->field_capacity,
                     &ecs->fields);
    nu__slotlist_init(
        alloc, NU_MEMORY_USAGE_ECS, info->field_capacity, &ecs->field_list);
    if (info->load_names)
    {
        ecs->field_names = nu__alloc(alloc,
                                     sizeof(nu_ident_t) * info->field_capacity,
                                     NU_MEMORY_USAGE_ECS);
    }
    ecs->first_field = NU_SLOT_NULL;

    /* archetypes */
    nu__slotmap_init(alloc,
                     NU_MEMORY_USAGE_ECS,
                     sizeof(nu__archetype_entry_t)
                         + sizeof(nu__archetype_field_t)
                               * (info->field_capacity - 1),
    ...
    ecs->archetype_bits  = nu__archetype_bits_alloc(
        alloc, info->field_capacity, info->archetype_capacity);
...
{
    nu__slot_t it = ecs->first_field;
    while (it)
    {
        nu__component_entry_t *entry = nu__slotmap_get(ecs->fields, it);
        if (entry->uid == uid)
        {
            return it;
        }
        it = nu__slotlist_next(ecs->field_list, it);
    }
    return NU_SLOT_NULL;
}
static nu_error_t
                            ...
                            nu__slot_t                *slot)
{
    ...
    /* insert component */
    *slot             = nu__slotmap_add(ecs->fields);
    ...
    {
        nu_ident_set_str(ecs->field_names[nu_slot_index(*slot)], info->name);
    }

    nu__slotlist_add_first(ecs->field_list, &ecs->first_field, *slot);
...
static void *
nu__ecs_field (nu__ecs_t *ecs, nu_entity_t e, nu_field_t f)
{
    nu_u16_t               group, index;
    nu__group_entry_t     *entry;
    nu__archetype_field_t *component;
